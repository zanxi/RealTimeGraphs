    ...
    , ui(new Ui::MainWindow)
{
    ...
    //genbigdata = new GenerateDBData(this);
    //genbigdata->start(QThread::NormalPriority);
    ...
    //connect(mTimer, SIGNAL(timeout()),this, SLOT(plotRandom()));
    //connect(mTimer, SIGNAL(timeout()),this, SLOT(plotRandom2()));
    ...
    //for(int i=0; i< threadCount;i++)
    {
        ...
        //jobs.append(new Worker(thStart,thEnd,step));
    }
...
void MainWindow::setupAdvancedAxesDemo(QCustomPlot *customPlot)
{
    ...
    customPlot->yAxis->setTickLabels(false);
    connect(customPlot->yAxis2, SIGNAL(rangeChanged(QCPRange)), customPlot->yAxis, SLOT(setRange(QCPRange))); // left axis only mirrors inner right axis
    connect(customPlot->yAxis3, SIGNAL(rangeChanged(QCPRange)), customPlot->yAxis2, SLOT(setRange(QCPRange))); // left axis only mirrors inner right axis
    customPlot->yAxis2->setVisible(true);
    customPlot->yAxis3->setVisible(true);
    customPlot->axisRect()->addAxis(QCPAxis::atRight);
    customPlot->axisRect()->axis(QCPAxis::atRight, 0)->setPadding(30); // add some padding to have space for tags
    ...
    mGraph1->setPen(QPen(QColor(250, 120, 0)));
    mGraph2->setPen(QPen(QColor(0, 180, 60)));
    //mGraph3->setPen(QPen(QColor(198, 180, 0)));
    ...
    //dateTicker->setDateTimeFormat("HH:mm:ss d. MMMM\nyyyy");
    //dateTicker->setDateTimeFormat("MMMM d - HH:mm:ss");
...
bool MainWindow::fromStringValue(const QString &inStr, QDateTime &value) {
...
void MainWindow::tabwidget_insert_add_column(QSqlQuery *query, QTableWidget *tab, QString nameColumnX,  int numRow, int numColumn)
{
    //QString const DateTimeFormat = "yyyy-MM-ddTHH:mm:ss[Z|[+|-]HH:mm]";
    QString const DateTimeFormat = "yyyy-MM-ddTHH:mm:ss.zzzZ";
    //QString const DateTimeFormat = "yyyy-MM-ddTHH:mm:ss.zzzZ";
    ...
    if(nameColumnX.contains("timey1"))
    {
    ...
    if(nameColumnX.contains("paramtime1"))
    {
       ...
       //a.fromString(t_,DateTimeFormat);
       //a.fromString(t_, Qt::ISODate);
       //QDateTime time2 = QDateTime::fromString(t_, Qt::ISODateWithMs).toLocalTime();
       QDateTime time2 = QDateTime::fromString(t_, Qt::ISODateWithMs).currentDateTimeUtc();
       xi = time2.toTime_t();
       //fromStringValue(t_,a);
       logger::WriteMsg("date: "+a.toString().toStdString());
       //a.fromString(t_, Qt::ISODateWithMs);
...
void MainWindow::GetData_search(QString id)
{
...
void MainWindow::GetData_search(QString id, QString tab, QString str2)
{
    //DataSystems::Instance().log+="id: "+id+;
    logger::WriteLog("Begin ID:"+QString::number(this->id));

    ui->textEdit->setText(DataSystems::Instance().log);
    ui->textEdit_2->setText(
        "Begin read date: "+DataSystems::Instance().time_read+ "\n" +
        "id: "+QString::number(this->id)+ "\n" +
                            ...
                            "coord_teat_rf min: "+QString::number(min3)+"\n" +
                            "coord_teat_rf max: "+QString::number(max3)+"\n"
                            );
    //ui->textEdit->;
    //ui->te
    ...
    if(db_func->StatusOpen())
    {
        ...
        logger::WriteLog(" ------- rows: " + QString::number(query->size()));


        if(ui->tableWidget->rowCount()>20)ui->tableWidget->clear();
...
void MainWindow::TimeOperation()
{
}

// SELECT * FROM public.dataentry_robot ORDER BY id ASC LIMIT 1

void MainWindow::GetData_FirstId(QString id, QString tab, QString str2)
{
    ...
    if(db_func->StatusOpen())
    {
        ...
        while(query->next())
        {
            ...
            QString t_ = query->value("paramtime1").toString();
            QDateTime time2 = QDateTime::fromString(t_, Qt::ISODateWithMs).currentDateTimeUtc();
            DataSystems::Instance().time_read = time2.toString("yyyy-MM-dd hh:mm:ss.zzzZ");
            logger::WriteLog("Begin read date: "+DataSystems::Instance().time_read);
...
void MainWindow::TimeOperation(QCustomPlot *customPlot)
{
...
void MainWindow::plotRandom_()
{
    ...
    mGraph1->setName("coord_teat_lf");
    mGraph2->setName("coord_teat_lr");
    //mGraph3->setName("coord_teat_rf");
    //mGraph3->addData(cow_graph1_x,cow_graph3_y);
    //mGraph1->set;
    //mGraph2->addData(x, y2);
...
void MainWindow::addPoint(double x, double y, double z)
{
    cow_graph1_x.append(x);
    cow_graph1_y.append(y);
...
void MainWindow::addPoint(double x, double y, double z, double w)
{
    cow_graph1_x.append(x);
    cow_graph2_x.append(x);
    cow_graph3_x.append(x);
    cow_graph1_y.append(y);
    cow_graph2_y.append(z);
    cow_graph3_y.append(w);
}

void MainWindow::addPoint_x1_5(double x, double y, double z, double w)
{
    x0.append(x);
    y0.append(y);
